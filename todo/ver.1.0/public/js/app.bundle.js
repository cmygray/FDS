...
var Axios = function () {
  ...
  _createClass(Axios, [{
    ...
    value: function post(url, obj) {
      ...
      return new Promise(function (res, rej) {
        var req = new XMLHttpRequest();
        req.open('POST', '' + (_this2.origin + url));
    ...
    value: function _delete(url) {
      ...
      return new Promise(function (res, rej) {
        var req = new XMLHttpRequest();
        req.open('DELETE', '' + (_this3.origin + url));
        req.send();
        req.onreadystatechange = function () {
          if (req.readyState === XMLHttpRequest.DONE) {
            if (req.status === 200) {
    ...
    key: 'patch',
    value: function patch(url, obj) {
      var _this4 = this;

      return new Promise(function (res, rej) {
        var req = new XMLHttpRequest();
        req.open('PATCH', '' + (_this4.origin + url));
        req.setRequestHeader('Content-type', 'application/json');
        req.send(JSON.stringify(obj));
        req.onreadystatechange = function () {
          if (req.readyState === XMLHttpRequest.DONE) {
...
(function () {
  ...
  var filterTodo = function filterTodo() {
    var status = document.querySelector('.active').id;
    if (status === 'active') {
      return todos.filter(function (todo) {
  ...
  var checkTodo = function checkTodo(id, completed) {
    var pl = { completed: completed };
    _axios2.default.patch('/todos/' + id, pl).then(getTodos).catch(function (err) {
      return console.log(err);
    });
  };
  var toggleMarkAll = function toggleMarkAll(completed) {
    _axios2.default.patch('/todos', completed).then(getTodos).catch(function (err) {
      return console.log(err);
    });
  };
  var clearTodo = function clearTodo() {
    _axios2.default.delete('/todos/completed').then(getTodos).catch(function (err) {
      return console.log(err);
    });
  };
  ...
  document.querySelector('#chk-allComplete').addEventListener('click', function (e) {
    ...
    toggleMarkAll({ completed: completed });
  });
  // clear btn event
  ...
  document.querySelector('.nav-pills').addEventListener('click', function (e) {
    if (e.target.nodeName === 'A') {
      document.querySelectorAll('.nav-pills > li').forEach(function (tab) {
        tab.className = '';
